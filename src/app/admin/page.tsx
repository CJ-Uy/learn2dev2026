...
export default function AdminPage() {
  ...
  const [tab, setTab] = useState<"users" | "org-requests">("users");
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/admin/org-requests").then((r) => r.ok ? r.json() : []),
    ]).then(([u, o]: [unknown, unknown]) => {
      ...
      {tab === "org-requests" && (
        <div>
          {orgRequests.length === 0
            ? <p className="text-slate-400">No pending org admin requests.</p>
            : (
